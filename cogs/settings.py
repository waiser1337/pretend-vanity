...
class settings(commands.Cog):
    ...
    async def keeprole(self, ctx, bool_value: bool):
        ...
        try:
            ...
            if result:
                cursor.execute("UPDATE servers SET keeprole = %s WHERE guild_id = %s", (bool_value, guild.id))
                self.conn.commit()
                message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Keep role **option updated*** to `{bool_value}`."
            else:
                cursor.execute("INSERT INTO servers (guild_id, keeprole) VALUES (%s, %s)", (guild.id, bool_value))
                self.conn.commit()
    ...
    async def vanity(self, ctx, vanity: str):
        ...
        try:
            ...
            if result:
                cursor.execute("UPDATE servers SET vanity = %s WHERE guild_id = %s", (vanity, guild.id))
                self.conn.commit()
                message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Vanity **updated** to `{vanity}`."
            else:
                cursor.execute("INSERT INTO servers (guild_id, vanity) VALUES (%s, %s)", (guild.id, vanity))
                self.conn.commit()
    ...
    async def notifications(self, ctx, bool_value: bool):
        ...
        try:
            ...
            if result:
                cursor.execute("UPDATE servers SET notif = %s WHERE guild_id = %s", (bool_value, guild.id))
                self.conn.commit()
                message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Notifications **updated** to `{bool_value}`."
            else:
                cursor.execute("INSERT INTO servers (guild_id, notif) VALUES (%s, %s)", (guild.id, bool_value))
                self.conn.commit()
    ...
    async def vanitylock(self, ctx, bool_value: bool):
        ...
        try:
            ...
            if result:
                cursor.execute("UPDATE servers SET vanitylock = %s WHERE guild_id = %s", (bool_value, guild.id))
                self.conn.commit()
                message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Vanity lock **updated** to `{bool_value}`."
            else:
                cursor.execute("INSERT INTO servers (guild_id, vanitylock) VALUES (%s, %s)", (guild.id, bool_value))
                self.conn.commit()
    ...
    async def channel(self, ctx, notif_channel: discord.TextChannel):
        ...
        if notif_channel:
            if ctx.guild.me.guild_permissions.manage_roles and notif_channel.permissions_for(guild.me).send_messages and notif_channel.permissions_for(guild.me).embed_links:
                cursor = self.conn.cursor()
                try:
                    cursor.execute("SELECT * FROM servers WHERE guild_id = %s", (guild.id,))
                    result = cursor.fetchone()
                    ...
                    if result:
                        cursor.execute("UPDATE servers SET notif_channel = %s WHERE guild_id = %s", (notif_channel.id, guild.id))
                        self.conn.commit()
                        message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Notification **channel set** to {notif_channel.mention}."
                    else:
                        cursor.execute("INSERT INTO servers (guild_id, notif_channel) VALUES (%s, %s)", (guild.id, notif_channel.id))
                        self.conn.commit()
                        message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Notification **channel set** to {notif_channel.mention}."
        ...
        if role:
            if ctx.guild.me.guild_permissions.manage_roles:
                ...
                try:
                    ...
                    if result:
                        cursor.execute("UPDATE servers SET role_id = %s WHERE guild_id = %s", (role.id, guild.id))
                        self.conn.commit()
                        message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Role **updated** to {role.mention}."
                    else:
                        cursor.execute("INSERT INTO servers (guild_id, role_id) VALUES (%s, %s)", (guild.id, role.id))
                        self.conn.commit()
                        message = f"<:approve:1266758547128062005> <@{ctx.author.id}>: Set {role.mention} as an **award role**."

                    cursor.execute("SELECT vanity FROM servers WHERE guild_id = %s", (guild.id,))
                    result = cursor.fetchone()

                    if result:
                        vanity = result[0] or ""  # Default to empty string if vanity is None

                        for member in guild.members:
                            current_status = member.activity.name if member.activity else "None"
                            has_vanity = vanity.lower() in current_status.lower() if current_status else False
    ...
    async def settings(self, ctx):
        ...
        cursor.close()
        if result:
            vanity, role_id, notif_channel_id, keeprole = result[1], result[2], result[3], result[4]
            ...
            embed.add_field(name="Role", value=f"<@&{role.id}>" if role else "None", inline=True)
            embed.add_field(name="Notification Channel", value=f"<#{notif_channel.id}>" if notif_channel else "None", inline=True)
