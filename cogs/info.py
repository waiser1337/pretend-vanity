...
class info(commands.Cog):
    ...
    async def stats(self, ctx):
        ...
        cursor.execute("SELECT vanity FROM servers WHERE guild_id = %s", (ctx.guild.id,))
        ...
        if result:
            ...
            for member in ctx.guild.members:
                ...
                if has_vanity:
                    count += 1
        embed = Embed(description=f"Currently, **there** are `{count}` people **with** the vanity in their **status**.", color=0x729BB0)
